 ...
 *如果需要辉光效果，请务必关闭硬件加速，参考： http://blog.chenming.info/blog/2012/09/18/android-hardware-accel/
 */
public class MoliTimer extends View {
    ...
    private void initialize(Canvas canvas) {
        ...
        timeRemain = CalendarUtil.getEmptyCalendarGreenwich();
        timeStart = CalendarUtil.getEmptyCalendarGreenwich();
    ...
    public MoliTimer(Context context) {
        super(context);
    }

    //use this view in .xml file will invoke this constructor
    //在.xml中使用此控件时调用此构造函数
    public MoliTimer(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public MoliTimer(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //initialize timer using specified time invoker provide
        //按照调用者的指定时间初始化倒计时
        if (millis != 0){
